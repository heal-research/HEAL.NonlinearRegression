...
namespace HEAL.Expressions {
  public class GradientVisitor : ExpressionVisitor {
    public Expression<Expr.ParametricGradientFunction> Visit(Expression<Expr.ParametricFunction> expr) {
      var gParam = Expression.Parameter(typeof(double[]), "grad");

      return Expression.Lambda<Expr.ParametricGradientFunction>(Visit(expr.Body),
        expr.Parameters.Concat(new[] { gParam }));
    }
